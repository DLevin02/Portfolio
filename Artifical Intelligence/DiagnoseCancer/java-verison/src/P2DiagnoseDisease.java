...
public class P2DiagnoseDisease {
    ...
    public static void main(String[] args) {
        ...
        try (FileWriter writer = new FileWriter(outputPath)) {
            List<ArrayList<Integer>> trainData = cleanAndTransformData(trainPath);

            importData(trainData, writer);
            entropyFunctionAtRoot(trainData, writer);
            decisionStump(trainData, 8, writer);

            int[] featureArray = {3, 4, 6, 2, 5, 8};
            DecTreeNode rootWithFeature = treeFeature(trainData, featureArray);
            printSideways(rootWithFeature, "");

            printTree(rootWithFeature, writer);
            ...
            rootMaxDepth = buildTreeWithFeatureAndDepth(trainData, featureArray, 6, 0);
    ...
    private static void importData(List<ArrayList<Integer>> trainData, FileWriter writer) throws IOException {
        int benignCount = 0;
    ...
    private static void entropyFunctionAtRoot(List<ArrayList<Integer>> trainData, FileWriter writer) throws IOException {
     ...
     * It returns a pointer to the root node.
     */
    private static DecTreeNode treeFeature(List<ArrayList<Integer>> dataSet, int[] featureArray) {
        int numData = dataSet.size();
        int bestAttr = -1;
        int bestThreshold = Integer.MIN_VALUE;
        double bestScore = Double.NEGATIVE_INFINITY;
        ...
        if (!leaf) {
            ...
            if (!leaf) {
                node = new DecTreeNode(-1, bestAttr, bestThreshold);
                node.left = treeFeature(leftList, featureArray);
                node.right = treeFeature(rightList, featureArray);
            }
        }
     ...
     * It returns a pointer to the root node.
     */
    private static void decisionStump(List<ArrayList<Integer>> dataSet, int featureStump, FileWriter writer) throws IOException {
        int bestThreshold = Integer.MIN_VALUE;
        double bestScore = Double.NEGATIVE_INFINITY;
